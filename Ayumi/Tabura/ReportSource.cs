    ...
    public class ReportSource {
        ...
        public Boolean IsAbleToMergeBetweenRow(Int32 columnIndex, Int32 rowIndex, Int32 comparingRowIndex) {
            for (Int32 colIdx = 0; colIdx <= columnIndex; colIdx++) {
                Column currentColumn = GetColumnByIndex(colIdx);
                if (currentColumn.IsAbleToMerge == false)
                    continue;
                Cell cell = Rows[rowIndex].Cells[colIdx];
                Cell comparingCell = Rows[comparingRowIndex].Cells[colIdx];
        ...
        public Boolean IsAbleToMergeBetweenColumn(Int32 rowIndex, Int32 columnIndex, Int32 comparingColumnIndex) {
            Column currentColumn = GetColumnByIndex(columnIndex);
            Column comparingColumn = GetColumnByIndex(comparingColumnIndex);
            ...
            Cell comparingCell = Rows[rowIndex].Cells[comparingColumnIndex];
        ...
        Boolean IsReportKeyEqual(Row row1, Row row2) {
            ...
            foreach (Column keyCol in listReportKeyColumn) {
        ...
        void AddMeasureColumn(ref Row resultKey, Row sourceRow) {
            List<Column> listMeasureCol = GetMeasureColumn();
            foreach (Column measureCol in listMeasureCol) {
        ...
        void AddToSubTotalValue(Int32 idxRow) {
            foreach (String key in SubTotalKeys) {
                var currentSubTotalValue = new List<Double>();
        ...
        void CummulativeValueOfMeasureColumn(Int32 idxRow, ref List<Double> result) {
            List<Column> listMeasure = GetMeasureColumn();
            FillList<Double>(ref result, listMeasure.Count);

            Int32 idx = 0;
            foreach (Column measureCol in listMeasure) {
        ...
        void AddSubTotalToRowIfAvailable(Int32 idxRow, ref List<Row> result) {
            ...
            foreach (String key in SubTotalKeys) {
                Row lastRow = Rows[lastIdxRow];
                Row currRow = Rows[idxRow];
        ...
        void AddSubTotalFromLastKey(String key, Int32 idxRow, ref List<Row> result) {
            ...
            for (Int32 idx = SubTotalKeys.Count - 1; idx >= idxKey; idx--) {
                Row rowResult = GetSubTotalRowData(SubTotalKeys[idx], idxRow);
        ...
        Row GetSubTotalRowData(String key, Int32 idxRow) {
            var result = (Row) GetDataRow(idxRow - 1).Clone();
        ...
        void FillMeasureWithValue(ref Row result, List<Double> listValue) {
            List<Column> listMeasureColumn = GetMeasureColumn();
            Int32 idx = 0;
            foreach (Column measureCol in listMeasureColumn) {
        ...
        void FillWithTextGrandTotal(ref Row result) {
            List<Column> listReportKey = GetReportKeyColumn();
            foreach (Column reportKeyColumn in listReportKey) {
        ...
        public void AddColumnToHeader() {
            ...
            foreach (KeyValuePair<String, Column> col in Columns.ToList()) {
